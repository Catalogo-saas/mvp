...
function CartPanel({ c, title }: { c: BabyStorefrontController; title: string }) {
  ...
  return <div className={styles.shade} onMouseDown={(event) => { if (event.target === event.currentTarget) c.closeCart(); }}><aside className={styles.panel} role="dialog" aria-modal="true" aria-label="Tu carrito"><div className={styles.panelInner}><button className={styles.close} type="button" onClick={c.closeCart} aria-label="Cerrar carrito"><X /></button><h2 className={styles.cartTitle}>{title}</h2><div className={styles.cartLines}>{c.cart.length ? c.cart.map((item) => <article key={item.lineId}><div>{item.imageUrl ? <img src={item.imageUrl} alt={item.productName} /> : <span className={styles.imageFallback}><ImageIcon /></span>}</div><section><strong>{item.productName}</strong><small>{item.optionLabels.join(" · ") || "Sin variantes"}</small><span className={styles.quantity}><button type="button" onClick={() => c.updateQuantity(item.lineId, -1)}><Minus /></button><b>{item.quantity}</b><button type="button" onClick={() => c.updateQuantity(item.lineId, 1)}><Plus /></button></span></section><b>{formatMoney(item.unitPrice * item.quantity)}</b></article>) : <p className={styles.empty}>Tu carrito está vacío.</p>}</div>{c.store.freeShippingEnabled && c.cart.length ? <div className={styles.shipping}>{c.shippingRemaining > 0 ? `Te faltan ${formatMoney(c.shippingRemaining)} para el envío gratis.` : "¡Tu pedido tiene envío gratis!"}<span><i style={{ width: `${c.shippingProgress}%` }} /></span></div> : null}<div className={styles.total}><span>Total</span><b>{formatMoney(c.cartTotal)}</b></div><form className={styles.checkoutForm} onSubmit={c.submitOrder}><fieldset><legend>Datos de contacto</legend><label>Nombre completo<input name="customerName" autoComplete="name" placeholder="Nombre y apellido" minLength={2} required /></label><label>Teléfono<input name="customerPhone" type="tel" inputMode="numeric" autoComplete="tel" placeholder="381 123-4567" pattern="[0-9]{3} [0-9]{3}-[0-9]{4}" maxLength={12} onChange={(event) => { event.currentTarget.value = formatArgentineLocalPhone(event.currentTarget.value); }} required /></label></fieldset><fieldset><legend>Método de entrega</legend><div className={styles.twoChoices}><label className={choice(c.fulfillmentMethod === "pickup")}><input type="radio" checked={c.fulfillmentMethod === "pickup"} onChange={() => c.setFulfillmentMethod("pickup")} />Retiro</label><label className={choice(c.fulfillmentMethod === "delivery")}><input type="radio" checked={c.fulfillmentMethod === "delivery"} onChange={() => c.setFulfillmentMethod("delivery")} />Envío</label></div>{c.fulfillmentMethod === "delivery" ? <label>Domicilio<input name="deliveryAddress" autoComplete="street-address" placeholder="Calle, número, piso y localidad" minLength={5} required /></label> : <div className={styles.pickupAddress}><strong>Dirección para retirar el pedido</strong><span>{c.store.address || "A coordinar con el comercio"}</span></div>}</fieldset><fieldset><legend>Método de pago</legend><div className={styles.twoChoices}><label className={choice(c.paymentMethod === "cash")}><input type="radio" checked={c.paymentMethod === "cash"} onChange={() => c.setPaymentMethod("cash")} />Efectivo</label>{c.store.acceptTransferPayments ? <label className={choice(c.paymentMethod === "transfer")}><input type="radio" checked={c.paymentMethod === "transfer"} onChange={() => c.setPaymentMethod("transfer")} />Transferencia</label> : null}</div>{c.paymentMethod === "transfer" && c.store.acceptTransferPayments ? <PaymentDetails c={c} /> : null}</fieldset>{c.error ? <p className={styles.error}>{c.error}</p> : null}<button className={styles.whatsapp} disabled={c.loading || !c.cart.length || !c.store.availability.isOpen}><img src="/whatsapp.svg" alt="" aria-hidden="true" />{c.loading ? "Creando pedido..." : c.store.availability.isOpen ? "Enviar pedido" : "Tienda cerrada"}</button></form></div></aside></div>;
}
